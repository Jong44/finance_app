...
export async function POST(req: NextRequest, res: NextResponse) {
    try {
        ...
        try {
            ...
            const prepocessingImage = await sharp(buffer)
                .resize({ width: 1200 })
                .sharpen()
                .normalize()
                .grayscale()
            ...
            const { text, raw } = await detectTextFromImage(prepocessingImage);
            ...
            return NextResponse.json({
                success: false,
                reason: `Error processing image: ${processingError.message}`
            });
        }
