...
const Invoice = () => {
  ...
  const handleScanInvoice = async () => {
    ...
    try {
      ...
      console.log(
        `Uploading image: ${selectedImage.name}, size: ${selectedImage.size} bytes, type: ${selectedImage.type}`
      );
      ...
      if (result.success && result.data) {
        ...
        if (expenseFormRef.current) {
          ...
          const formData = {
            ...
            expense_details: expanse_detail.map((item: any) => ({
              ...
              category_budget: item.category_budget || "Wants",
        ...
        toast("Scanning failed", {
          description:
            result.reason || "Could not extract data from the invoice",
          // variant: "destructive"
        });
        ...
        if (
          result.reason &&
          result.reason.includes("not recognized as an invoice")
        ) {
          toast("Scanning tip", {
            description:
              "Try using a clearer image or a different invoice format",
          });
        } else if (
          result.reason &&
          result.reason.includes("No text could be detected")
        ) {
          toast("Scanning tip", {
            description:
              "The image quality is too low. Try taking a clearer photo with good lighting",
          });
        }
      }
    } catch (error: any) {
